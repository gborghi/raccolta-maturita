---
title: Liceo Comunicazione 2014 — Problema 2 — Problema 1
tipo: problema
item_id: prob_2014_liceo_della_comunicazione_2014_problema2_68_1
prova_id: prova_2014_liceo_della_comunicazione_2014_problema2_68
anno: '2014'
pdf: Prova_Maturita_2014.pdf
cluster: Studio di Funzione
numero: '1'
tags:
  - maturita/problema
  - area/analisi
  - cluster/studio_di_funzione
  - anno/2014
---

Fonte: p.68 · [📄 Apri PDF](https://drive.google.com/file/d/1WH26K6aDhT0rm439hBww1SX_Dwrvky9g/view)
**Prova:** [[Prove/2014_liceo_della_comunicazione_2014_problema2_68|Liceo Comunicazione 2014 — Problema 2]] · **Cluster:** [[Clusters/Studio di Funzione|Studio di Funzione]]

Sia $f(x) = x^3 + 3x^2 + 4x - 12$.

1. Si dimostri che l'equazione $f(x) = 0$ ha una sola radice $\alpha$ tale che $1 < \alpha < 2$.

2. Si verifichi che l'espressione $x = \sqrt{\dfrac{4(3-x)}{3+x}}$ equivale a $f(x) = 0$.
Posto $x_0 = 1{,}3$ si calcolino, con l'aiuto di una calcolatrice, $x_1, x_2, x_3, \dots$ ove è $x_{n+1} = \sqrt{\dfrac{4(3-x_n)}{3+x_n}}$. Cosa si può osservare? Si può congetturare che $x_n$, al crescere di $n$, approssimi sempre meglio il valore di $\alpha$? In che modo? Con quali considerazioni?

3. Sia $R$ la regione del quarto quadrante compresa fra il grafico $K$ di $f(x)$ e gli assi del sistema di coordinate $Oxy$. Si calcoli l'area di $R$.

4. Si introduca un nuovo sistema di riferimento ottenuto da $Oxy$ traslando gli assi e portando $O$ nel punto di flesso di $K$. Qual è l'equazione di $K$ nel nuovo sistema di riferimento?

**Topic:** [[Topics/Studio di Funzione|Studio di Funzione]], [[Calcolo Integrale]], [[Successioni e Serie]], [[Equazioni e Disequazioni]]
**Metodi:** [[Studio di funzione completo]], [[Teoremi di esistenza (continuita, Weierstrass, valori intermedi, zeri)]], [[Calcolo di aree e volumi con integrali (solidi di rotazione)]], [[Risoluzione di equazioni e disequazioni]]
**Competenze:** [[Dimostrazione e argomentazione rigorosa]], [[Ragionamento logico-deduttivo]], [[Calcolo simbolico e manipolazione algebrica]]
**Tipo funzione:** [[Polinomiale]]
**Soluzione:** [[Svolgimenti/sol_prob_2014_liceo_della_comunicazione_2014_problema2_68_1|📝 Svolgimento]] · [📄 PDF p.68](https://drive.google.com/file/d/1WH26K6aDhT0rm439hBww1SX_Dwrvky9g/view)


#maturita/problema #area/analisi #cluster/studio_di_funzione
