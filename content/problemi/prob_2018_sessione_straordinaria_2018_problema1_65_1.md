---
title: 2018 Sessione straordinaria — Problema 1 — Problema 1
tipo: problema
item_id: prob_2018_sessione_straordinaria_2018_problema1_65_1
prova_id: prova_2018_sessione_straordinaria_2018_problema1_65
anno: '2018'
...
**Prova:** [[Prove/2018_sessione_straordinaria_2018_problema1_65|2018 Sessione straordinaria — Problema 1]] · **Cluster:** [[Clusters/Studio di Funzione|Studio di Funzione]]
...
1. Stabilisci, motivando adeguatamente la risposta, quale delle seguenti funzioni può rappresentare adeguatamente il profilo della parte superiore della candela:

   $$\text{1.}\quad y=\begin{cases}\sqrt{a-x} & \text{se } 0\le x\le a\\[4pt]\sqrt{a+x} & \text{se } -a\le x<0\end{cases}\qquad \text{con } a\in\mathbb{R},\ a>0$$

   $$\text{2.}\quad y=a-x^2 \quad \text{in } \left[-\sqrt{a};\sqrt{a}\right]\qquad \text{con } a\in\mathbb{R},\ a>0$$

   $$\text{3.}\quad y=-\sqrt{|x|}+a \quad \text{in } \left[-a^2;a^2\right]\qquad \text{con } a\in\mathbb{R},\ a>0$$

   Utilizzando l'espressione analitica trovata, studia eventuali punti di singolarità del profilo della parte superiore della candela.

2. Per consentire l'inserimento dello stoppino al vertice della candela, è necessario che l'angolo $\vartheta$ in figura 1 non sia maggiore di $30°$. Determina di conseguenza i possibili valori del parametro $a$.

3. Attribuendo all'altezza e al raggio della parte cilindrica i valori rispettivamente di 8 e 2, in un'opportuna unità di misura, determina il volume totale della candela. Da questo dato dipenderanno il peso e il costo di produzione della candela stessa.

   Il produttore deve inscatolare le candele in confezioni da 3 e da 4 candele, posizionando le candele in verticale, con le basi circolari disposte in modo da occupare il minor spazio possibile. Si prevedono due possibili configurazioni per posizionare le basi circolari delle candele all'interno delle scatole, rappresentate in figura 3. *(figura nel PDF)*
...
**Soluzione:** [[Svolgimenti/sol_prob_2018_sessione_straordinaria_2018_problema1_65_1|📝 Svolgimento]] · [📄 PDF p.65](https://drive.google.com/file/d/1vt-OZDFhSWqupGghcan_yIZCMQ0DrJTR/view)
