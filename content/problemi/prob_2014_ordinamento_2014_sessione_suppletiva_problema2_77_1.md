---
title: Ordinamento 2014 Suppletiva — Problema 2 — Problema 1
tipo: problema
item_id: prob_2014_ordinamento_2014_sessione_suppletiva_problema2_77_1
prova_id: prova_2014_ordinamento_2014_sessione_suppletiva_problema2_77
anno: '2014'
pdf: Prova_Maturita_2014.pdf
cluster: Studio di Funzione
numero: '1'
tags:
  - maturita/problema
  - area/analisi
  - cluster/studio_di_funzione
  - anno/2014
---

Fonte: p.77 · [📄 Apri PDF](https://drive.google.com/file/d/1WH26K6aDhT0rm439hBww1SX_Dwrvky9g/view)
**Prova:** [[Prove/2014_ordinamento_2014_sessione_suppletiva_problema2_77|Ordinamento 2014 Suppletiva — Problema 2]] · **Cluster:** [[Clusters/Studio di Funzione|Studio di Funzione]]

Si consideri la funzione:
$$f(x) = \frac{1}{x\,\ln^2 x}.$$

1. Si studi tale funzione e si tracci il suo grafico $\gamma$, su un piano riferito ad un sistema di assi cartesiani ortogonali $Oxy$.

2. Si scriva l'equazione della tangente a $\gamma$ nel punto di ascissa $x = e$, e si calcoli l'area del trapezio $T$ che essa forma con l'asse $x$, con l'asintoto verticale e con la retta di equazione $x = e$.

3. Si calcoli l'area della regione $S$ delimitata dalla curva $\gamma$, dall'asse $x$ e dalle rette di equazioni $x = e$ ed $x = k$ (con $k > e$).

4. Si faccia vedere che $S$ tende verso un limite finito quando $k$ tende a $+\infty$ e si confronti tale limite col valore numerico dell'area del trapezio $T$, arrotondato alla quarta cifra decimale.

**Topic:** [[Topics/Studio di Funzione|Studio di Funzione]], [[Calcolo Integrale]], [[Esponenziali e Logaritmi]], [[Limiti e Continuita]]
**Metodi:** [[Studio di funzione completo]], [[Derivazione e calcolo di derivate]], [[Integrazione (calcolo di integrali)]], [[Calcolo di limiti e continuita]]
**Competenze:** [[Calcolo simbolico e manipolazione algebrica]], [[Lettura e interpretazione di grafici e funzioni]], [[Ragionamento logico-deduttivo]]
**Tipo funzione:** [[Logaritmica]], [[Mista  trascendente composita|Mista / trascendente composita]]
**Soluzione:** [[Svolgimenti/sol_prob_2014_ordinamento_2014_sessione_suppletiva_problema2_77_1|📝 Svolgimento]] · [📄 PDF p.77](https://drive.google.com/file/d/1WH26K6aDhT0rm439hBww1SX_Dwrvky9g/view)


#maturita/problema #area/analisi #cluster/studio_di_funzione
