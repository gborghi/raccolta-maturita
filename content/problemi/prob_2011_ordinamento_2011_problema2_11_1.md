---
title: 2011 Ordinamento — Problema 2 — Problema 1
tipo: problema
item_id: prob_2011_ordinamento_2011_problema2_11_1
prova_id: prova_2011_ordinamento_2011_problema2_11
anno: '2011'
pdf: Prova_Maturita_2011.pdf
cluster: Studio di Funzione
numero: '1'
tags:
  - maturita/problema
  - area/analisi
  - cluster/studio_di_funzione
  - anno/2011
---

Fonte: p.11 · [📄 Apri PDF](https://drive.google.com/file/d/1n7qQSR6joIJerX8l_7M5ZnD0O28Aqabx/view)
**Prova:** [[Prove/2011_ordinamento_2011_problema2_11|2011 Ordinamento — Problema 2]] · **Cluster:** [[Clusters/Studio di Funzione|Studio di Funzione]]

È data la funzione
$$f(x)=(ax+b)\,e^{-\frac{x}{3}}+3,$$
dipendente dai parametri reali $a$ e $b$.

1) Si determinino $a$ e $b$ in modo che il grafico di $f$ passi per il punto $(0\,;\,2)$ e presenti in $x=4$ un punto a tangente orizzontale, verificando che risulta $a=1$ e $b=-1$.

2) Posto dunque $f(x)=(x-1)\,e^{-\frac{x}{3}}+3$, si studi tale funzione (dominio, intersezioni con gli assi, segno, limiti ed eventuali asintoti, crescenza e decrescenza con massimi e minimi, concavità ed eventuali flessi) e se ne tracci il grafico.

3) Si calcoli l'area della regione di piano compresa tra il grafico di $f$ e l'asintoto orizzontale $y=3$.

4) Si supponga che $f(x)$ rappresenti l'andamento del profitto (in milioni di euro) di un'azienda in funzione del tempo $x$. Verificato, mediante la tabella dei valori, che la funzione è accettabile come modello, si stabilisca a quale valore non potrà essere inferiore il profitto al crescere indefinito di $x$.

**Topic:** [[Topics/Studio di Funzione|Studio di Funzione]], [[Esponenziali e Logaritmi]], [[Calcolo Integrale]], [[Derivate]]
**Metodi:** [[Studio di funzione completo]], [[Derivazione e calcolo di derivate]], [[Calcolo di aree e volumi con integrali (solidi di rotazione)]], [[Modellizzazione matematica di un problema realeapplicato|Modellizzazione matematica di un problema reale/applicato]]
**Competenze:** [[Modellizzazione matematica di situazioni]], [[Lettura e interpretazione di grafici e funzioni]], [[Interpretazione e formalizzazione del testo]]
**Tipo funzione:** [[Mista  trascendente composita|Mista / trascendente composita]], [[Dipendente da un parametro]]
**Soluzione:** [[Svolgimenti/sol_prob_2011_ordinamento_2011_problema2_11_1|📝 Svolgimento]] · [📄 PDF p.11](https://drive.google.com/file/d/1n7qQSR6joIJerX8l_7M5ZnD0O28Aqabx/view)


#maturita/problema #area/analisi #cluster/studio_di_funzione
