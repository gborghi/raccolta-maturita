---
title: 2010 Estero Australe Ordinaria — Problema 1 — Problema 1
tipo: problema
item_id: prob_2010_estero_calendario_australe_ordinaria_2010_problema1_45_1
prova_id: prova_2010_estero_calendario_australe_ordinaria_2010_problema1_45
anno: '2010'
pdf: Prova_Maturita_2010.pdf
cluster: Studio di Funzione
numero: '1'
tags:
  - maturita/problema
  - area/analisi
  - cluster/studio_di_funzione
  - anno/2010
---

Fonte: p.45 · [📄 Apri PDF](https://drive.google.com/file/d/1J5aZuRvV3aZZ5NwuxHjhKtmXNI-5yTxd/view)
**Prova:** [[Prove/2010_estero_calendario_australe_ordinaria_2010_problema1_45|2010 Estero Australe Ordinaria — Problema 1]] · **Cluster:** [[Clusters/Studio di Funzione|Studio di Funzione]]

Sia $f$ la funzione di dominio $I = \left]-\frac{1}{2};\,+\infty\right[$ definita da $f(x) = \ln(1 + 2x)$.

**a)** Qual è il codominio di $f$? Si dimostri che $f$ è strettamente crescente su $I$ e se ne tracci il grafico $\gamma$.

**b)** Sia $g(x) = f(x) - x$ con $x \in I$; si studi come varia $g(x)$ su $I$.

**c)** Si dimostri che l'equazione $g(x) = 0$ ammette due soluzioni: $0$ e un'altra, denotata con $\beta$, appartenente all'intervallo $[1, 2]$. Si dimostri altresì che per tutti gli $x$ reali dell'intervallo $J = \,]0;\,\beta[$ anche $f(x)$ appartiene a $J$.

**d)** Si calcoli l'area della parte di piano delimitata dal grafico di $f$ e dall'asse $x$ sull'intervallo $[0, 2]$.

**Topic:** [[Topics/Studio di Funzione|Studio di Funzione]], [[Esponenziali e Logaritmi]], [[Calcolo Integrale]], [[Derivate]]
**Metodi:** [[Studio di funzione completo]], [[Derivazione e calcolo di derivate]], [[Calcolo di aree e volumi con integrali (solidi di rotazione)]], [[Dimostrazioneprova rigorosa di una proprieta|Dimostrazione/prova rigorosa di una proprieta]]
**Competenze:** [[Dimostrazione e argomentazione rigorosa]], [[Lettura e interpretazione di grafici e funzioni]], [[Calcolo simbolico e manipolazione algebrica]]
**Tipo funzione:** [[Logaritmica]]
**Soluzione:** [[Svolgimenti/sol_prob_2010_estero_calendario_australe_ordinaria_2010_problema1_45_1|📝 Svolgimento]] · [📄 PDF p.45](https://drive.google.com/file/d/1J5aZuRvV3aZZ5NwuxHjhKtmXNI-5yTxd/view)


#maturita/problema #area/analisi #cluster/studio_di_funzione
