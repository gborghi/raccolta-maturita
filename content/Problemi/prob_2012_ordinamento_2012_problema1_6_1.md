---
title: 2012 Ordinamento — Problema 1 — Problema 1
tipo: problema
item_id: prob_2012_ordinamento_2012_problema1_6_1
prova_id: prova_2012_ordinamento_2012_problema1_6
anno: '2012'
pdf: Prova_Maturita_2012.pdf
cluster: Studio di Funzione
numero: '1'
tags:
  - maturita/problema
  - area/analisi
  - cluster/studio_di_funzione
  - anno/2012
---

Fonte: p.6 · [📄 Apri PDF](https://drive.google.com/file/d/15uTp-FXyM3r6gZHrCu6HThex3dLEYCIU/view)
**Prova:** [[Prove/2012_ordinamento_2012_problema1_6|2012 Ordinamento — Problema 1]] · **Cluster:** [[Clusters/Studio di Funzione|Studio di Funzione]]

Siano $f$ e $g$ le funzioni definite, per tutti gli $x$ reali, da

$$f(x)=|27x^3| \qquad e \qquad g(x)=\operatorname{sen}\!\left(\frac{3}{2}\pi x\right)$$

**1)** Fissato un conveniente sistema di riferimento cartesiano $Oxy$, si studino $f$ e $g$ e se ne disegnino i rispettivi grafici. Si verifichi, in particolare, che $f$ è una funzione pari e si determini il periodo $T$ di $g$.

**2)** Si scrivano le equazioni delle rette $r$ e $s$ tangenti, rispettivamente, ai grafici di $f$ e di $g$ nel punto di coordinate $\left(\frac{1}{3};\,1\right)$. Quale è l'ampiezza, in gradi e primi sessagesimali, dell'angolo acuto formato da $r$ e da $s$?

**3)** Si calcoli l'area della regione $R$ racchiusa tra i grafici di $f$ e di $g$ nell'intervallo $0 \le x \le \frac{1}{3}$.

**4)** La regione $R$ è la base di un solido. Si scrivano, spiegandone il perché ma senza calcolarli, gli integrali definiti che forniscono i volumi dei solidi $S$ e $T$ ottenuti dalla rotazione di $R$, rispettivamente, attorno all'asse $x$ e attorno all'asse $y$.

**Topic:** [[Topics/Studio di Funzione|Studio di Funzione]], [[Trigonometria]], [[Calcolo Integrale]], [[Geometria Solida]]
**Metodi:** [[Studio di funzione completo]], [[Derivazione e calcolo di derivate]], [[Calcolo di aree e volumi con integrali (solidi di rotazione)]], [[Uso di relazioni e identita trigonometriche]]
**Competenze:** [[Lettura e interpretazione di grafici e funzioni]], [[Calcolo simbolico e manipolazione algebrica]], [[Visualizzazione spaziale e geometrica]]
**Tipo funzione:** [[Con valore assoluto  modulo|Con valore assoluto / modulo]], [[Goniometrica  trigonometrica|Goniometrica / trigonometrica]]
**Soluzione:** [[Svolgimenti/sol_prob_2012_ordinamento_2012_problema1_6_1|📝 Svolgimento]] · [📄 PDF p.6](https://drive.google.com/file/d/15uTp-FXyM3r6gZHrCu6HThex3dLEYCIU/view)


#maturita/problema #area/analisi #cluster/studio_di_funzione
