---
title: 2010 Estero Santiago Ordinaria — Problema 1 — Problema 1
tipo: problema
item_id: prob_2010_estero_santiago_del_cile_ordinaria_2010_problema1_145_1
prova_id: prova_2010_estero_santiago_del_cile_ordinaria_2010_problema1_145
anno: '2010'
pdf: Prova_Maturita_2010.pdf
cluster: Studio di Funzione
numero: '1'
tags:
  - maturita/problema
  - area/analisi
  - cluster/studio_di_funzione
  - anno/2010
---

Fonte: p.145 · [📄 Apri PDF](https://drive.google.com/file/d/1J5aZuRvV3aZZ5NwuxHjhKtmXNI-5yTxd/view)
**Prova:** [[Prove/2010_estero_santiago_del_cile_ordinaria_2010_problema1_145|2010 Estero Santiago Ordinaria — Problema 1]] · **Cluster:** [[Clusters/Studio di Funzione|Studio di Funzione]]

Sia $f$ la funzione definita da: $f(x)=(x^2+1)e^{-x+2}$, con $x\in\mathbb{R}$.

**a)** Nel piano riferito ad un sistema di riferimento cartesiano, si disegni il grafico $\lambda$ di $f(x)$.

**b)** Si provi che la retta di equazione $y=\dfrac{5}{2}x$ interseca $\lambda$ nel punto $Q$ di ascissa $2$. Qual è l'equazione della retta tangente a $\lambda$ in $Q$?

**c)** Sia $g(x)=(-x^2-2x-3)e^{-x+2}$. Si calcoli $g'(x)$ e si deduca da essa una primitiva di $f(x)$.

**d)** Si calcoli l'area della regione del primo quadrante delimitata da $\lambda$, dall'asse $x$ e dalla retta $x=2$ e con l'aiuto di una calcolatrice se ne dia un valore approssimato arrotondato ai centesimi.

**Topic:** [[Topics/Studio di Funzione|Studio di Funzione]], [[Esponenziali e Logaritmi]], [[Derivate]], [[Calcolo Integrale]]
**Metodi:** [[Studio di funzione completo]], [[Derivazione e calcolo di derivate]], [[Integrazione (calcolo di integrali)]], [[Impostazione con geometria analitica (coordinate, coniche)]]
**Competenze:** [[Lettura e interpretazione di grafici e funzioni]], [[Calcolo simbolico e manipolazione algebrica]], [[Calcolo numerico e stima]]
**Tipo funzione:** [[Esponenziale]]
**Soluzione:** [[Svolgimenti/sol_prob_2010_estero_santiago_del_cile_ordinaria_2010_problema1_145_1|📝 Svolgimento]] · [📄 PDF p.145](https://drive.google.com/file/d/1J5aZuRvV3aZZ5NwuxHjhKtmXNI-5yTxd/view)


#maturita/problema #area/analisi #cluster/studio_di_funzione
