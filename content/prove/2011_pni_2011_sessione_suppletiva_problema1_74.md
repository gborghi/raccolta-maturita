...
1) Si calcoli il rapporto $\dfrac{\overline{BE}+\overline{DF}}{\overline{AB}}$, espresso in funzione di $x=B\hat{A}E$, controllando che risulta:
$$f(x)=\operatorname{tg}x+\operatorname{cotg}x.$$

2) Si studi la funzione $f(x)$ e si tracci il suo grafico $\gamma$ nell'intervallo $0\le x\le\pi$.

3) Si calcoli l'area della superficie piana, delimitata dalla curva $\gamma$ e dalla retta di equazione $y=\dfrac{4}{3}\sqrt{3}$.
