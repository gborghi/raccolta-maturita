...
Sono date la famiglia di funzioni
$$f(x) = ax^3 + x, \quad \text{con } a \in \mathbb{R},$$
e la funzione
$$g(x) = x^4 - x^2.$$

a) Si dimostri che tutti i grafici delle funzioni $f$ hanno in comune un unico punto e che tale punto è comune anche al grafico di $g$. Si eseguano poi lo studio di $f(x)$ al variare di $a$ e lo studio di $g(x)$.

b) Si dimostri che l'equazione $ax^3 + x = x^4 - x^2$ ammette almeno due soluzioni qualunque sia il valore del parametro $a$.

c) Si determini per quale valore di $a$ i grafici della funzione $f$ e della funzione $g$ ammettono la stessa tangente nel punto di ascissa $-1$. Si rappresentino i grafici delle due funzioni e la retta tangente comune.

d) Con riferimento al punto precedente, si calcoli l'area della regione finita di piano delimitata dai grafici delle due funzioni.
...
Nel riferimento cartesiano $xOy$ si consideri la funzione $f(x)$ definita e continua in $]-4;\, +\infty[$, e con la derivata seconda $f''(x)$ avente il grafico riportato nella figura.

*(figura nel PDF)*

È noto che la retta di equazione $x = -4$ è un asintoto per $f(x)$ e che la tangente al grafico di $f(x)$ nel punto di ascissa $x = 0$ è parallela alla bisettrice del I e del III quadrante, mentre la tangente nel punto di ascissa $-3$ ha equazione $4x - y + 14 = 0$.

a) Si determini l'area della parte di piano delimitata dal grafico di $f''(x)$ e dall'asse $x$ nell'intervallo $[-3;\, 0]$.

b) Si tracci un grafico probabile per $f'(x)$.

c) Sapendo che $f''(x) = -\dfrac{4}{(x+4)^2}$ e utilizzando tutte le informazioni già assegnate, si determini l'espressione analitica di $f(x)$ e, dopo averne completato lo studio, si rappresenti il suo grafico $\Gamma$.

d) Si calcoli l'area della parte di piano compresa tra $\Gamma$ e l'asse $x$ nell'intervallo $[-3;\, 0]$.
...
1. Dati due insiemi $A$ e $B$, costituiti rispettivamente da 6 e da 5 elementi tutti distinti, si determini il numero di funzioni suriettive da $A$ a $B$ che è possibile definire.
...
5. È data una funzione $f(x)$, continua e derivabile con derivata puntualmente minore della funzione esponenziale $y = e^x$ (con $e$ base del logaritmo naturale); si dimostri che: $f(1) - f(0) < e$.
...
6. Dato il cubo in figura, si determini l'ampiezza del diedro convesso che ha per spigolo $BD'$ e per facce i piani $A'D'B$ e $C'D'B$. *(figura nel PDF)*
...
8. Sia data la funzione: $g(x) = \displaystyle\int_x^{2x} f(t)\, dt$, con $f(t)$ continua su $\mathbb{R}$. Sapendo che $f(0) = -\dfrac{1}{2}$, $f\left(\dfrac{1}{2}\right) = \dfrac{1}{4}$ e $f(1) = 1$, si calcolino $g(0)$, $g'(0)$ e $g'\left(\dfrac{1}{2}\right)$.
