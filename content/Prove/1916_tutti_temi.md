---
tipo: prova
prova_id: prova_1916_tutti_temi
anno: '1916'
pdf: tutti_temi.pdf
cluster: Geometria
tags:
  - maturita/prova
  - area/geometria
  - cluster/geometria
  - anno/1916
title: 1916 Tema di maturita
---

Fonte: p.62 · [📄 Apri PDF](https://drive.google.com/file/d/1TfImhTiR0dAKm4PiIDPxk3_ptWdJ4Pko/view)
**Cluster:** [[Clusters/Geometria|Geometria]]

## Problema 1

*↳ Nota atomica: [[Problemi/prob_1916_tutti_temi_2|Problema 1]]*

**Problema 1**

I tre spigoli $x$, $y$, $z$ d'un parallelepipedo rettangolo formano, nell'ordine in cui sono scritti, una progressione aritmetica.

Data la somma $a$ dei tre spigoli e sapendo che la somma delle tre facce da essi formate è equivalente al quadrato di lato $b$, calcolare $x$, $y$, $z$. Discussione.

**Topic:** [[Geometria Solida]], [[Successioni e Serie]], [[Equazioni e Disequazioni]]
**Metodi:** [[Geometria solida nello spazio (volumi, superfici, sezioni)]], [[Sistemi di equazioni]], [[Risoluzione di equazioni e disequazioni]], [[Discussione di famiglie di curvefunzioni parametriche|Discussione di famiglie di curve/funzioni parametriche]]
**Competenze:** [[Calcolo simbolico e manipolazione algebrica]], [[Problem solving (strategia risolutiva non immediata)]], [[Visualizzazione spaziale e geometrica]]
**Fonte:** [📄 Testo (PDF) — p.62](https://drive.google.com/file/d/1TfImhTiR0dAKm4PiIDPxk3_ptWdJ4Pko/view)

## Problema 1

*↳ Nota atomica: [[Problemi/prob_1916_tutti_temi_4|Problema 1]]*

**Problema 1**

Di una piramide regolare quadrangolare sono date:

1. la somma $a$ delle lunghezze dell'apotema e del doppio del lato di base;
2. l'area $b^2$ della superficie totale.

Determinare la lunghezza del lato di base e quella dell'apotema.

**Topic:** [[Geometria Solida]], [[Equazioni e Disequazioni]]
**Metodi:** [[Impostazione con geometria analitica (coordinate, coniche)]], [[Sistemi di equazioni]], [[Risoluzione di equazioni e disequazioni]]
**Competenze:** [[Calcolo simbolico e manipolazione algebrica]], [[Problem solving (strategia risolutiva non immediata)]], [[Ragionamento logico-deduttivo]]
**Fonte:** [📄 Testo (PDF) — p.62](https://drive.google.com/file/d/1TfImhTiR0dAKm4PiIDPxk3_ptWdJ4Pko/view)

## Problema 2

*↳ Nota atomica: [[Problemi/prob_1916_tutti_temi_1|Problema 2]]*

**Problema 2**

Sono dati un numero reale $m$ ed una circonferenza di centro $O$ e di diametro $AB$. Si chiami $C$ il punto medio del raggio $\overline{OB}$. Dire se esiste sulla semiretta $OA$ un punto $D$ da cui, tirata una tangente alla circonferenza, e detti $E$ il punto di contatto ed $F$ la proiezione ortogonale di $E$ su $AB$, risulti uguale ad $m$ il rapporto tra il volume del solido generato dal triangolo $DEC$ in una sua rotazione completa intorno ad $AB$ ed il volume del cono avente per base il cerchio dato e per altezza il segmento $\overline{AF}$.

Dimostrare che se risultasse $F$ il punto medio di $\overline{DC}$, si avrebbe uguale a

$$\frac{-1 + \sqrt{33}}{8}$$

il seno dell'angolo $\widehat{EDA}$.

**Topic:** [[Geometria Solida]], [[Geometria Euclidea]], [[Equazioni e Disequazioni]]
**Metodi:** [[Geometria solida nello spazio (volumi, superfici, sezioni)]], [[Ottimizzazione ricerca di massimi e minimi|Ottimizzazione: ricerca di massimi e minimi]], [[Uso di relazioni e identita trigonometriche]], [[Risoluzione di equazioni e disequazioni]]
**Competenze:** [[Ottimizzazione e ricerca dell'ottimo]], [[Calcolo simbolico e manipolazione algebrica]], [[Problem solving (strategia risolutiva non immediata)]]
**Fonte:** [📄 Testo (PDF) — p.62](https://drive.google.com/file/d/1TfImhTiR0dAKm4PiIDPxk3_ptWdJ4Pko/view)

## Problema 2

*↳ Nota atomica: [[Problemi/prob_1916_tutti_temi_3|Problema 2]]*

**Problema 2**

Sui lati $OX$, $OY$, d'un angolo retto sono dati rispettivamente i punti $A$ e $B$. Determinare un punto $C$ del segmento $\overline{OA}$ e un punto $D$ del segmento $\overline{OB}$, in modo che siano eguali i tre segmenti $\overline{AC}$, $\overline{CD}$, $\overline{DB}$.

**Topic:** [[Geometria Euclidea]], [[Equazioni e Disequazioni]]
**Metodi:** [[Geometria solida nello spazio (volumi, superfici, sezioni)]], [[Successioni, serie e ricorrenze]], [[Sistemi di equazioni]], [[Discussione di famiglie di curvefunzioni parametriche|Discussione di famiglie di curve/funzioni parametriche]]
**Competenze:** [[Calcolo simbolico e manipolazione algebrica]], [[Problem solving (strategia risolutiva non immediata)]], [[Ragionamento logico-deduttivo]]
**Fonte:** [📄 Testo (PDF) — p.62](https://drive.google.com/file/d/1TfImhTiR0dAKm4PiIDPxk3_ptWdJ4Pko/view)

## Problema 2

*↳ Nota atomica: [[Problemi/prob_1916_tutti_temi_5|Problema 2]]*

**Problema 2**

D'un triangolo rettangolo sono date:

1. la somma $a$ delle lunghezze dell'ipotenusa e dell'altezza ad essa relativa;
2. la differenza $b$ fra le lunghezze delle proiezioni dei cateti sull'ipotenusa.

Determinare le lunghezze dei lati.

**Topic:** [[Geometria Euclidea]], [[Equazioni e Disequazioni]]
**Metodi:** [[Geometria euclidea sintetica (similitudine, teoremi su triangolicerchi)|Geometria euclidea sintetica (similitudine, teoremi su triangoli/cerchi)]], [[Sistemi di equazioni]], [[Risoluzione di equazioni e disequazioni]]
**Competenze:** [[Calcolo simbolico e manipolazione algebrica]], [[Problem solving (strategia risolutiva non immediata)]], [[Ragionamento logico-deduttivo]]
**Fonte:** [📄 Testo (PDF) — p.62](https://drive.google.com/file/d/1TfImhTiR0dAKm4PiIDPxk3_ptWdJ4Pko/view)


#maturita/prova #area/geometria #cluster/geometria
