...
2. Si supponga che $f(x)$ costituisca, ovviamente in opportune unità di misura, il modello di crescita di un certo tipo di popolazione. Quali informazioni sulla sua evoluzione si possono dedurre dai grafici in figura e in particolare dal fatto che $f(x)$ presenta un asintoto orizzontale e un punto di flesso?

3. Se $\Gamma$ è il grafico della funzione $f(x)=\dfrac{a}{1+e^{bx}}$, si provi che $a=8$ e $b=2$.
...
1. Si studi $f$ e si tracci il suo grafico $\gamma$ su un piano riferito ad un sistema di assi cartesiani ortogonali ortometrici $Oxy$; accertato che $\gamma$ presenta sia un punto di flesso che un punto di minimo se ne calcolino, con l'aiuto di una calcolatrice, le ascisse arrotondate alla terza cifra decimale.
