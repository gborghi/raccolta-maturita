...
Si trovi:
$$\lim_{x\to 3^-}f(x); \qquad \lim_{x\to 3^+}f(x); \qquad \lim_{x\to 3}f(x).$$
...
Sia $D$ la regione finita di piano delimitata dalla curva d'equazione $y=\sqrt{\sin x}$ e dall'asse $x$ nell'intervallo $0\le x\le\pi$. Si calcoli il volume del solido generato da $D$ in una rotazione completa attorno all'asse delle $x$.
