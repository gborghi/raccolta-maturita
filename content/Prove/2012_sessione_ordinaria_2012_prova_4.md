...
2. Si scrivano le equazioni delle rette $r$ e $s$ tangenti, rispettivamente, a $G_f$ e a $G_g$ nel punto di ascissa $x=\frac{1}{3}$. Qual è l'ampiezza, in gradi e primi sessagesimali, dell'angolo acuto formato da $r$ e da $s$?
...
2. La regione $R$ è la base di un solido $W$ le cui sezioni, ottenute tagliando $W$ con piani perpendicolari all'asse $x$, hanno, per ogni $0\le x\le 3$, area $S(x)=e^{5-3x}$. Si determini il volume di $W$.
...
10. Quale delle seguenti funzioni è positiva per ogni $x$ reale?
    ...
    A) $\operatorname{sen}(\cos(x^2+1))$  B) $\cos(\operatorname{sen}(x^2+1))$  C) $\ln(\operatorname{sen}(x^2+1))$  D) $\cos(\ln(x^2+1))$
