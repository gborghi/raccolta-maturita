...
1. Stabilisci in quali punti del suo insieme di definizione la funzione $f$ è continua e in quali è derivabile e verifica l'esistenza dei limiti: $\displaystyle\lim_{x\to+\infty} f(x)$ e $\displaystyle\lim_{x\to+\infty}\frac{f(x)}{x}$; qualora esistano, determinane il valore. Rappresenta inoltre, per $x\in[0;4]$, i grafici delle funzioni:
$$g(x)=f'(x)\ ,\qquad h(x)=\int_{0}^{x} f(t)\,dt.$$

2. Considera la funzione: $s(x)=\operatorname{sen}(bx)$ con $b$ costante reale positiva; determina $b$ in modo che $s(x)$ abbia lo stesso periodo di $f(x)$. Dimostra che la porzione quadrata di piano $OABC$ in figura 1 viene suddivisa dai grafici di $f(x)$ e $s(x)$ in 3 parti distinte e determina le probabilità che un punto preso a caso all'interno del quadrato $OABC$ ricada in ciascuna delle 3 parti individuate.
