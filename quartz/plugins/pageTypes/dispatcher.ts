...
function populateVirtualPageHtmlAst(
  ...
  for (const ve of virtualEntries) {
    ...
    try {
      ...
      // NOTE: do NOT do `ve.tree.children = htmlAst.children` here. That mutates
      // the emit tree with the rendered body HTML, so FolderContent/TagContent
      // then renders its listing once as its own content AND again as pageBody
      // (every folder/tag page listing appears twice). We only need htmlAst for
      // transclusion — keep that, drop the tree mutation.
