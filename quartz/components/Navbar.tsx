...
//   • a fixed left RAIL — collapsed to icons, expandable (with a toggle) to a
//     labelled drawer; it is the single section index.
//   • a fixed TOPBAR (brand + relocated Quartz toolbar: search / theme / reader).
// All layout/styling lives in quartz/styles/custom.scss (.m-rail / .m-topbar …);
// the driver script (nav height, rail expand/collapse, active link) is inlined in
// quartz/plugins/emitters/componentResources.ts. Links are ABSOLUTE, built from
// the configured baseUrl's path so they carry the GitHub-Pages project base on
// every page.
...
// the rail is the complete section index (icons collapsed; labels when expanded).
...
const Navbar: QuartzComponent = ({ cfg, displayClass }: QuartzComponentProps) => {
  ...
  return (
    <div class={classNames(displayClass, "m-shell")}>
      {/* ── left rail: icon-only when collapsed, icon+label when expanded ── */}
      <aside class="m-rail" aria-label="Navigazione">
        <a class="m-rail-logo" href={`${bp}/`} aria-label="Home" title="Home">M</a>
        {/* expand / collapse toggle — driver persists the state in localStorage */}
        <button
          type="button"
          class="m-rail-toggle"
          data-rail-toggle
          ...
          title="Espandi menu"
        >
          <span class="m-rail-ic" dangerouslySetInnerHTML={{ __html: svg("expand") }} />
          <span class="m-rail-lbl">Comprimi</span>
        </button>
        {RAIL.map(([label, slug, icon]) => (
          <a class="m-rail-btn" href={href(slug)} title={label} aria-label={label} data-nav-slug={slug}>
            <span class="m-rail-ic" dangerouslySetInnerHTML={{ __html: svg(icon) }} />
            <span class="m-rail-lbl">{label}</span>
          </a>
        ))}
        ...
        <button
          ...
          hidden
        >
          <span class="m-rail-ic" dangerouslySetInnerHTML={{ __html: svg("grafo") }} />
          <span class="m-rail-lbl">Vista grafo</span>
        </button>
        <a
          class="m-rail-btn m-rail-gh"
          ...
          aria-label="GitHub"
        >
          <span class="m-rail-ic" dangerouslySetInnerHTML={{ __html: svg("github", true) }} />
          <span class="m-rail-lbl">GitHub</span>
        </a>
      </aside>
